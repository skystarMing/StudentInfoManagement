...
public class DeleteStudentByDepartmentFram extends JFrame {
    //系部标签
    JLabel lbl_department;
    //系部文本框
    JTextField txt_department;
    //面板
    ...
    public DeleteStudentByDepartmentFram(String title) throws HeadlessException {
        super(title);
        initGUI();
    }
    private void initGUI() {
        ...
        colHead=new Vector();
        //标签与文本框
        lbl_department=new JLabel("输入系部");
        txt_department=new JTextField(10);
        txt_department.setHorizontalAlignment(JTextField.CENTER);
        //按钮
        ...
        pnlNorth.add(lbl_department);
        pnlNorth.add(txt_department);
        ...
        table.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                //设置文本框内容
                txt_department.setText(table.getValueAt(row,4).toString());
    ...
    private void doQuery() {
        String clazz=txt_department.getText().trim();
        if (!clazz.equals("")){
            students=  studentService.findStudentsByDepartment(clazz);
            if (students.size()>0){
                //让删除键可用
